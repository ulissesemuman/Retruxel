...
namespace Retruxel.Target.GG.Toolchain;

/// <summary>
/// Game Gear toolchain implementation using devkitSMS + SDCC.
/// Extracts embedded binaries to %AppData%\Retruxel\toolchain\gg\ on first run.
/// Compiles with -DTARGET_GG flag and produces .gg ROM files.
/// </summary>
public class GgToolchain : IToolchain
{
    private static readonly string ToolchainPath = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
        "Retruxel", "toolchain", "gg");

    public string TargetId => "gg";
    public string DisplayName => "devkitSMS + SDCC 4.5.24 (Game Gear)";
    ...
    public async Task ExtractAsync(IProgress<string> progress)
    {
        ...
        progress.Report("EXTRACTING: GG toolchain binaries...");
        ...
        foreach (var resource in resources
            .Where(r => r.StartsWith("Retruxel.Target.GG")))
        {
            var parts = resource.Split('.');
            var ext = parts[^1];
            var nameWithoutExt = string.Join(".", parts[..^1]);
            var fileName = nameWithoutExt
                .Replace("Retruxel.Target.GG.Toolchain.Resources.", "")
        ...
        progress.Report("DONE: GG toolchain ready.");
    }
    ...
    public async Task<BuildResult> BuildAsync(BuildContext context, IProgress<string> progress)
    {
        ...
        try
        {
            ...
            // Step 3 — Convert .ihx to .gg (ROM goes to build root)
