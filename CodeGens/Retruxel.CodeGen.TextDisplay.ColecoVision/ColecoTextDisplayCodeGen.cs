...
namespace Retruxel.CodeGen.TextDisplay.ColecoVision;
...
/// ColecoVision uses the same devkitSMS/SMSlib as Master System.
/// </summary>
public class ColecoTextDisplayCodeGen : ICodeGenPlugin
{
    ...
    public IEnumerable<string> Validate(string moduleJson)
    {
        ...
        if (x < 0 || x > MaxX)
            yield return $"text.display: x={x} exceeds ColecoVision limit ({MaxX}). Text will render off-screen.";

        if (y < 0 || y > MaxY)
            yield return $"text.display: y={y} exceeds ColecoVision limit ({MaxY}). Text will render off-screen.";
    ...
    {
        var assembly = typeof(ColecoTextDisplayCodeGen).Assembly;
        var resourceName = "Retruxel.CodeGen.TextDisplay.ColecoVision.Templates.text_display.c.rtrx";
