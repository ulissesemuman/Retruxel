...
namespace Retruxel.Target.SG1000.Toolchain;

/// <summary>
/// SG-1000 toolchain implementation using devkitSMS + SDCC.
/// Extracts embedded binaries to %AppData%\Retruxel\toolchain\sg1000\ on first run.
/// Compiles with -DTARGET_SG1000 flag and produces .sg ROM files.
/// </summary>
public class SG1000Toolchain : IToolchain
{
    private static readonly string ToolchainPath = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
        "Retruxel", "toolchain", "sg1000");

    public string TargetId => "sg1000";
    public string DisplayName => "devkitSMS + SDCC 4.5.24 (SG-1000)";
    ...
    public async Task ExtractAsync(IProgress<string> progress)
    {
        ...
        progress.Report("EXTRACTING: SG-1000 toolchain binaries...");
        ...
        foreach (var resource in resources
            .Where(r => r.StartsWith("Retruxel.Target.SG1000")))
        {
            var parts = resource.Split('.');
            var ext = parts[^1];
            var nameWithoutExt = string.Join(".", parts[..^1]);
            var fileName = nameWithoutExt
                .Replace("Retruxel.Target.SG1000.Toolchain.Resources.", "")
        ...
        progress.Report("DONE: SG-1000 toolchain ready.");
    }

    /// <summary>
    /// Compiles generated C source files into a .sg ROM using SDCC + ihx2sms.
    /// </summary>
    public async Task<BuildResult> BuildAsync(BuildContext context, IProgress<string> progress)
    {
        ...
        try
        {
            ...
            // Step 3 — Convert .ihx to .sg (ROM goes to build root)
            var ihx2smsPath = Path.Combine(ToolchainPath, "ihx2sms.exe");
            var ihxPath = Path.Combine(srcDirectory, $"{romName}.ihx");
            var romPath = Path.Combine(context.OutputDirectory, $"{romName}.sg");
