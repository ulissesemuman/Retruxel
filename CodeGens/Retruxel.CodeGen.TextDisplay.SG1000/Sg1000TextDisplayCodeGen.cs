...
namespace Retruxel.CodeGen.TextDisplay.SG1000;

/// <summary>
/// SG-1000-specific code generator for the text.display module.
/// Uses template-based code generation from text_display.c.rtrx.
/// SG-1000 uses the same devkitSMS/SMSlib as Master System.
/// </summary>
public class Sg1000TextDisplayCodeGen : ICodeGenPlugin
{
    ...
    public IEnumerable<string> Validate(string moduleJson)
    {
        ...
        if (x < 0 || x > MaxX)
            yield return $"text.display: x={x} exceeds SG-1000 limit ({MaxX}). Text will render off-screen.";

        if (y < 0 || y > MaxY)
            yield return $"text.display: y={y} exceeds SG-1000 limit ({MaxY}). Text will render off-screen.";
    ...
    {
        var assembly = typeof(Sg1000TextDisplayCodeGen).Assembly;
        var resourceName = "Retruxel.CodeGen.TextDisplay.SG1000.Templates.text_display.c.rtrx";
