...
namespace Retruxel.Target.ColecoVision.Toolchain;

/// <summary>
/// ColecoVision toolchain implementation using devkitSMS + SDCC.
/// Extracts embedded binaries to %AppData%\Retruxel\toolchain\coleco\ on first run.
/// Compiles with -DTARGET_COLECO flag and produces .col ROM files.
/// </summary>
public class ColecoVisionToolchain : IToolchain
{
    private static readonly string ToolchainPath = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
        "Retruxel", "toolchain", "coleco");

    public string TargetId => "coleco";
    public string DisplayName => "devkitSMS + SDCC 4.5.24 (ColecoVision)";
    ...
    public async Task ExtractAsync(IProgress<string> progress)
    {
        ...
        progress.Report("EXTRACTING: ColecoVision toolchain binaries...");
        ...
        foreach (var resource in resources
            .Where(r => r.StartsWith("Retruxel.Target.ColecoVision")))
        {
            var parts = resource.Split('.');
            var ext = parts[^1];
            var nameWithoutExt = string.Join(".", parts[..^1]);
            var fileName = nameWithoutExt
                .Replace("Retruxel.Target.ColecoVision.Toolchain.Resources.", "")
        ...
        progress.Report("DONE: ColecoVision toolchain ready.");
    }

    /// <summary>
    /// Compiles generated C source files into a .col ROM using SDCC + ihx2sms.
    /// </summary>
    public async Task<BuildResult> BuildAsync(BuildContext context, IProgress<string> progress)
    {
        ...
        try
        {
            ...
            // Step 3 — Convert .ihx to .col (ROM goes to build root)
            var ihx2smsPath = Path.Combine(ToolchainPath, "ihx2sms.exe");
            var ihxPath = Path.Combine(srcDirectory, $"{romName}.ihx");
            var romPath = Path.Combine(context.OutputDirectory, $"{romName}.col");
