...
/// <summary>
/// NES toolchain using cc65 + neslib.
/// Extracts embedded binaries to %AppData%\Retruxel\toolchain\nes\ on first run.
///
/// Build pipeline:
...
public class NesToolchain : IToolchain
{
    ...
    public async Task ExtractAsync(IProgress<string> progress)
    {
        ...
        var resources = assembly.GetManifestResourceNames()
            .Where(r => r.StartsWith("Retruxel.Target.NES.Toolchain.Resources."));

        foreach (var resource in resources)
        {
            // Resource name format: Retruxel.Target.NES.Toolchain.Resources.bin.cc65.exe
            // → relative path: bin/cc65.exe
            var relativePath = resource
                .Replace("Retruxel.Target.NES.Toolchain.Resources.", "")
                .Replace('.', Path.DirectorySeparatorChar);

            // Restore correct extension — last segment after the last separator
            // (the replace above turns "cc65.exe" into "cc65/exe", need to fix)
            relativePath = FixResourcePath(resource);
    ...
    public async Task<BuildResult> BuildAsync(BuildContext context, IProgress<string> progress)
    {
        ...
        try
        {
            ...
            // Write generated source files to src directory
            ...
            var cc65  = Path.Combine(ToolchainPath, "bin", "cc65.exe");
            var ca65  = Path.Combine(ToolchainPath, "bin", "ca65.exe");
            var ld65  = Path.Combine(ToolchainPath, "bin", "ld65.exe");
            var crt0  = Path.Combine(ToolchainPath, "include", "crt0.s");
            var nesLib = Path.Combine(ToolchainPath, "lib", "nes.lib");
            var nesCfg = Path.Combine(ToolchainPath, "lib", "nes.cfg");
            var includeDir = Path.Combine(ToolchainPath, "include");

            // Create NES configuration file in src directory
            ...
            // ── Step 3: ld65 — link everything (ROM goes to build root) ───────
    ...
    public async Task<bool> VerifyAsync()
    {
        var required = new[]
        {
            Path.Combine(ToolchainPath, "bin",     "cc65.exe"),
            Path.Combine(ToolchainPath, "bin",     "ca65.exe"),
            Path.Combine(ToolchainPath, "bin",     "ld65.exe"),
            Path.Combine(ToolchainPath, "include", "crt0.s"),
            Path.Combine(ToolchainPath, "include", "neslib.h"),
            Path.Combine(ToolchainPath, "lib",     "nes.lib"),
            Path.Combine(ToolchainPath, "lib",     "nes.cfg"),
        };
    ...
    /// Handles the fact that dots in filenames (e.g. "cc65.exe") are
    /// indistinguishable from namespace separators in resource names.
    ///
    /// Convention used in the .csproj EmbeddedResource paths:
    ///   bin/cc65.exe    → Retruxel.Target.NES.Toolchain.Resources.bin.cc65.exe
    ///   include/nes.h   → Retruxel.Target.NES.Toolchain.Resources.include.nes.h
    ///   lib/nes.cfg     → Retruxel.Target.NES.Toolchain.Resources.lib.nes.cfg
    /// </summary>
    private static string FixResourcePath(string resourceName)
    {
        // Strip the base namespace prefix
        var relative = resourceName.Replace("Retruxel.Target.NES.Toolchain.Resources.", "");

        // The first segment is the subfolder (bin / include / lib)
        // Everything after is the filename — rejoin with OS separator for the folder,
        // but keep the last dot as the extension separator.
        var segments = relative.Split('.');
        if (segments.Length < 2) return relative;

        var folder    = segments[0];                          // "bin"
        var extension = segments[^1];                        // "exe" / "h" / "cfg" / "s" / "sinc"
        var baseName  = string.Join(".", segments[1..^1]);   // "cc65" / "nes" / ...
