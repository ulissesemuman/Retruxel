...
public class SmsTarget : ITarget
{
    ...
    public GeneratedFile GenerateMainFile(RetruxelProject project, IEnumerable<GeneratedFile> moduleFiles)
    {
        ...
        // Generate splash files if enabled
        var splashFiles = new List<GeneratedFile>();
        if (splashEnabled)
        {
            var splashGen = new SmsSplashCodeGen();
            splashFiles.Add(splashGen.GenerateCode());
            splashFiles.Add(splashGen.GenerateHeader());
            fileList.AddRange(splashFiles);

            // Rebuild headers after adding splash files
            headers = fileList
                .Where(f => f.FileType == GeneratedFileType.Header)
                .Select(f => $"#include \"{f.FileName}\"");
        }
        ...
        var content = string.Join("\n", [
            ...
            "",
            "void main(void) {",
            .. (splashEnabled ? new[] { "    sms_splash_show();" } : Array.Empty<string>()),
            .. initCalls,
            .. textDisplayCalls,
            "",
            "    while(1) {",
