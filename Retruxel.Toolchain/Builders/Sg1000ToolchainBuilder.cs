...
public class Sg1000ToolchainBuilder : IToolchainBuilder
{
    ...
    public string DisplayName => "SDCC + devkitSMS + SMSlib (SG-1000)";
    ...
    public async Task<BuildResult> BuildAsync(BuildContext context, IProgress<string> progress)
    {
        ...
        try
        {
            ...
            var crt0Path = Path.Combine(libPath, "crt0_sg.rel");
            var sglibLib = Path.Combine(libPath, "SGlib.lib");
            ...
            foreach (var file in sourceFiles)
            {
                ...
                var compileArgs = $"-mz80 --no-std-crt0 --sdcccall 1 -DTARGET_SG1000 " +
